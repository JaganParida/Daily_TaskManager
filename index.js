document.addEventListener("DOMContentLoaded", function () {
  ...
  const dismissNotification = document.getElementById("dismissNotification"); // Calendar elements
  ...
  const todayMonthBtn = document.getElementById("todayMonth"); // Helper function to get local date string in YYYY-MM-DD format
  ...
  taskDate.value = getLocalDateString(); // Check if user exists in localStorage

  let user = JSON.parse(localStorage.getItem("user")) || null;

  let tasks = JSON.parse(localStorage.getItem("tasks")) || [];

  let history = JSON.parse(localStorage.getItem("history")) || []; // Template tasks for daily recurrence

  let templateTasks = JSON.parse(localStorage.getItem("templateTasks")) || []; // Notification timeouts and intervals

  let notificationTimeouts = {};

  let notificationIntervals = {}; // Calendar state

  let currentMonth = new Date().getMonth();

  let currentYear = new Date().getFullYear(); // Theme cycle: system -> light -> dark

  const themeCycle = ["system", "light", "dark"];

  let currentThemeIndex = 0; // Initialize the app

  initApp(); // Event Listeners
  ...
  dismissNotification.addEventListener("click", dismissNotificationPermission); // Calendar navigation
  ...
  navBtns.forEach((btn) => {
    btn.addEventListener("click", function () {
      const sectionId = this.getAttribute("data-section"); // Update active button

      navBtns.forEach((b) => b.classList.remove("active"));

      this.classList.add("active"); // Show active section

      sections.forEach((section) => section.classList.remove("active"));

      document.getElementById(sectionId).classList.add("active"); // If history section is activated, update the days completed
  ...
  function initApp() {
    ...
    checkNewDay(); // Show welcome popup if no user data
    ...
    updateThemeIcon(savedTheme); // Check notification permission

    checkNotificationPermission(); // Load tasks and history

    renderTasks();

    renderHistory();

    updateDailyProgress(); // Schedule notifications for existing tasks
  ...
  function startApp() {
    ...
    if (name) {
      ...
      welcomePopup.style.display = "none"; // Check notification permission after user starts
  ...
  function resetApp() {
    if (
      ...
      templateTasks = []; // Clear all notifications
  ...
  function addTask() {
    ...
    if (title && date && time) {
      ...
      localStorage.setItem("tasks", JSON.stringify(tasks)); // Check if template already exists for this task
  ...
  function renderTasks() {
    tasksContainer.innerHTML = ""; // Get today's date in YYYY-MM-DD format

    const today = getLocalDateString(); // Filter tasks for today
...
                        `;
  ...
  function saveEditedTask() {
    ...
    if (title && date && time) {
      ...
      if (taskIndex !== -1) {
        ...
        updateDailyProgress(); // Schedule new notifications for the updated task
  ...
  function completeTask(taskId) {
    ...
    if (taskIndex !== -1) {
      tasks[taskIndex].completed = true; // Clear notifications for this task
  ...
  function deleteTask(taskId) {
    ...
    if (taskIndex !== -1) {
      const task = tasks[taskIndex]; // Remove from template tasks

      templateTasks = templateTasks.filter(
        (t) => !(t.title === task.title && t.time === task.time)
      );

      localStorage.setItem("templateTasks", JSON.stringify(templateTasks)); // Clear notifications for this task
...
                `;
    ...
    if ("Notification" in window) {
      Notification.requestPermission().then((permission) => {
        if (permission === "granted") {
          notificationPermission.style.display = "none"; // Schedule notifications for existing tasks
  ...
  function scheduleAllNotifications() {
    if (Notification.permission !== "granted") return; // Clear all existing notifications
    ...
    notificationIntervals = {}; // Schedule notifications for all uncompleted tasks
  ...
  function scheduleNotification(task) {
    ...
    const notifyTime = new Date(taskDateTime.getTime() - 30 * 60000); // Don't schedule if the notification time has already passed

    if (notifyTime < new Date()) {
      return;
    }

    const timeUntilNotification = notifyTime - new Date(); // Schedule the notification
  ...
  function renderCalendar() {
    ...
    calendarTitle.textContent = `${monthNames[currentMonth]} ${currentYear}`; // Clear previous calendar

    calendarGrid.innerHTML = ""; // Add weekday headers
    ...
    const startDay = firstDay.getDay(); // 0 = Sunday, 1 = Monday, etc. // Add empty cells for days before the first day of the month
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      dayElement.textContent = day; // Check if this is today
  ...
  setInterval(checkNewDay, 60000); // Initial calendar render
